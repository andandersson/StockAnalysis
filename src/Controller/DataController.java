...
public class DataController {
	...
	public int getDateNum(String date, StringBuffer dataStr, String[][] datArr) {
		/*
		 * returning the row in the nested array for where the startdate and stopdate
		 * are. It is looping through the first element in all rows. it is returning the
		 * number for the input, if there is no input, or if the input is in wrong
		 * format, it is returning 0
		 * 
		 */
	...
	public String[][] bestOpPric(int start, int stop, String[][] arr) {
		/*
		 * in this method, we are returning which dates had the best opening price
		 * compared to 5 days simple moving average (SMA 5)?
		 * 
		 */
			...
			if (choice == 1) {
				...
				if (startNum > 248) {
					...
					jop.showMessageDialog(f,
							"Unfortunately we are not able to count the range from this starting date, " + "\n"
